          ...
          <div className='text-xs font-bold relative'>
            <div className='absolute top-0 right-0 z-10'>
              <div className='flex items-center p-1 bg-white dark:bg-slate-800 text-slate-800 dark:text-white'>
                <FaStar className='mr-1' /> {this.props.rating.toFixed(1)}
              </div>
            </div>
            <img className='cursor-pointer' onClick={this.props.goToDetail} src={this.props.poster === undefined ? "https://via.placeholder.com/500x750?text=No+Image" : 'https://image.tmdb.org/t/p/w500' + this.props.poster} alt="" />
          </div>
          <div className=' my-2 text-sm '>
            <p onClick={this.props.goToDetail} className='cursor-pointer font-bold line-clamp-2'>{this.props.title}
            </p>
            <span onClick={this.props.goToDetail} className='cursor-pointer'> ({this.props.release ? this.props.release.split('-')[0] : ''})</span>
          </div>
        </div>
